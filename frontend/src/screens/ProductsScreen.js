...
function ProductsScreen(props) {
  ...
  return <div className="content content-margined">
    ...
    <div className="product-header">
      <h3>Products</h3>
      <button className="button primary" onClick={() => openModal({})}><span>Create Product</span></button>
    </div>
    {modalVisible &&
      <div className="form">
        <form onSubmit={submitHandler} >
          <ul className="form-container">
            ...
            <li>
              <button type="submit" className="button primary"><span>{id ? "Update" : "Create"}</span></button>
            </li>
            <li>
              <button type="button" onClick={() => setModalVisible(false)} className="button a">Back</button>
            </li>
          </ul>
        </form>
      </div>
        ...
        <tbody>
          {products.map(product => (<tr key={product._id}>
            ...
            <td>
              ...
              {' '}
              <button className="button b" onClick={() => deleteHandler(product)} >Delete</button>
            </td>
          </tr>))}
        </tbody>
      </table>
